...
class ChromeUtil {
  public static convertBookmarksToMarkdown(
    bookmarks: Bookmark[],
    level: number = 1,
    parent: string = ""
  ): string {
    ...
    for (const bookmark of bookmarks) {
      if (bookmark.type === "folder" && bookmark.children) {
        ...
        markdown += "\n" + "#".repeat(level) + " " + tag + "\n";
        markdown += ChromeUtil.convertBookmarksToMarkdown(
          bookmark.children,
          level + 1,
          tag
        );
  ...
  public static convertChromeBookmarksToMarkdown(bookmarkData: string): string {
    ...
    const bookmark_bar_md = ChromeUtil.convertBookmarksToMarkdown(
      [bookmark_bar],
      1,
      ""
    );
    const other_md = ChromeUtil.convertBookmarksToMarkdown([other], 1, "");
    const synced_md = ChromeUtil.convertBookmarksToMarkdown([synced], 1, "");
    const markdown = bookmark_bar_md + "\n" + other_md + "\n" + synced_md;
    return markdown;
  }

  public static testChromeUtil() {
    const originPath =
    ...
    const markdown = ChromeUtil.convertChromeBookmarksToMarkdown(fileData);
