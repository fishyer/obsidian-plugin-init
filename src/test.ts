...
export default class TestClient {
  ...
  public async testUrl2md(plugin: Plugin) {
    ...
    const fileContent = await dataAdapter.read("assets/inoreader-3631.md");
    //截取前30个链接
    const i=400;
    const j=i+50;
    ...
    executeTasks(links, outputDir, dataAdapter, bottomStatusBar);
...
async function executeTasks(links, outputDir, dataAdapter, bottomStatusBar) {
  for (let i = 0; i < links.length; i++) {
    const elapsedTime = Math.ceil((performance.now() - startTime) / 1000);
    bottomStatusBar.setText(`转换进度: ${i+1}/${links.length} 百分比: ${Math.ceil((i+1)/links.length*100)}% 耗时: ${elapsedTime}s`);
    const link = links[i];
    const url = link.url;
